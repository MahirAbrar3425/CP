namespace Geometry
{
 ...
 ///line
  struct line{
    ...
    line(pt v,T c){/*From direction vector v and offset c*/
      this->v=v,this->c=c;
    }
 ...
 /*int latticePoints(pt a,pt b){
  ...
  } // A = i+(b/2)-1; here A=area,i=pointsinside,b=pointsonline
  Polygon*/
  ...
  bool pointInPolygon(const vector<pt>&p,pt q){/*returns true if pt q is in polygon p*/
    bool c=false;
    for(int i=0,n=p.size();i<n;i++){
      int j=(i+1)%p.size();
  ...
  ll is_point_in_convex(vector<pt>& p, pt &x) { // O(log n)
      ll n = p.size(); /*this function from YouKnowWho*/
      if (n < 3) return 1;
...
//  if(scale<eps)return c;
 ...
 ///Circle
  ...
  int tangents(pt c1,double r1,pt c2,double r2,bool inner,vector<pair<pt,pt>>&out){
    if(inner)r2=-r2;/*returns tangent(the line which touch a circle in one point)of two circle*/
 ...
 //Convex Hull-Monotone Chain
  ...
  vector<pt>monotoneChain(vector<pt>&points){
    ...
    for(int i=0,sz=points.size();i<sz;i++){
      while(st>=2 and orient(H[st-2],H[st-1],points[i])<0)st--;
      H[st++]=points[i];
    }
    int taken=st-1;
    for(int i=points.size()-2;i>=0;i--){
      while(st>=taken+2 and orient(H[st-2],H[st-1],points[i])<0)st--;
      H[st++]=points[i];
    }
